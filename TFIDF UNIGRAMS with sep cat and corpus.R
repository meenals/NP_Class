...
#no of words per document: not done
#m<-vector()
#y<-vector()
#for(m in 1:length(news_test){
 # y=y+nchar(as.character(news_test[[m]]))
}
wordsperDoc<-sapply(news_test,FUN = function(x) sum(str_count(string = x, pattern = "\\w") + 1))
...
#create dtm matrix for test and train set with 0.99 sparsity with TFIDF and unigram
...
#for naive bayes, we do not make dictionary equal as we apply laplace smoothing, so we process below commands instead of above
...
#for ngrams ,TFIDF dtm matrix below commands are required
...
write.csv(as.data.frame(data.matrix(dtm_Train_TFIDF)),"dtm_Train_TFIDF.csv")
mydictionary<-dtm_Train_TFIDF$dimnames[["Terms"]]
#add dictionary to make test and train set vocabulary the same.
dtm_Test_TFIDF<-DocumentTermMatrix(news_test_cl,control=list(weighting=function(x)weightTfIdf(x,normalize=TRUE),dictionary=mydictionary))
dtm_Test_TFIDF$dimnames$Docs<-1:nrow(dtm_Test_TFIDF)
dtm_Test_TFIDF<-removeSparseTerms(dtm_Test_TFIDF,0.999)
dtm_Train_TFIDF$dimnames$Docs<-1:nrow(dtm_Test_TFIDF)

#for Naive bayes we do not need to make dictionary same so use below command
...
#find freq words and association generating word cloud( associations pending) 
...
#plotting word freq of words that occur more than50 times in the entire training set
Wordfreq <- sort(colSums(as.matrix(dtm_Train_TFIDF)), decreasing=TRUE) 
wordfreq_df <- data.frame(word = names(Wordfreq), freq =Wordfreq)   
ggplot(subset(wordfreq_df, freq>50), aes(word, freq))+ geom_bar(stat="identity",fill="blue")+theme(axis.text.x=element_text(angle=45, hjust=1))+labs(title="Words with TFIDF greater than 50 in Training set") +labs(x="Words", y="Frequency(TFIDF"))

#find freq words for each doc, one by one
#list_freqs <- lapply(dtm_Train_TFIDF$dimnames$Docs, 
 #                   function(i) findFreqTerms(as.matrix(dtm_Train_TFIDF)[dtm_Train_TFIDF$dimnames$Docs == i], 2, 100))
...
## Also tried tuning the minsplit, minbucket and cp parameters manually
...
#clustering with scaled matrix
distancematrix_docs<-dist(scale(as.matrix(dtm_Train_TFIDF_clust)))
...
splHier<-split(as.matrix(dtm_Train_TFIDF_clust),News_cluster_hier_scaled)
head(splHier)
NewsClusterCentreHierar<- lapply(splHier,colMeans)
dim(splHier)
length(splHier)
write.csv(NewsClusterCentreHierar,"NewsClusterCentreHierar.csv")
...
#Average values of each category in all the clusters
NG_ClusterPlusNG7avgValues<- t(aggregate(as.matrix(dtm_Train_TFIDF_clust),by=list(NG_Cluster$cluster),mean))[-1,]
avgValuesNG_df<-as.data.frame(NG_ClusterPlusNG7avgValues)
...
#pamk
library(fpc)
NewsFit_pamK<-pamk(as.matrix(dtm_Train_TFIDF_sp),metric="manhattan")
NoOfClusters<-NewsFit_pamK$nc# we get 4 clusters
NewsResultPam<-NewsFit_pamK$pamobject
# print cluster medoids
for (i in 1:NoOfClusters) { 
  cat(paste("cluster", NoOfClusters, ": "))
  cat(colnames(NewsResultPam$medoids)[which(NewsResultPam$medoids[i,]==1)], "\n")
  # print tweets in cluster i
  print(colnames(NewsResultPam$medoids)[NewsResultPam$clustering==i])
}


for (i in 1:4) { 
  cat(paste("cluster", i, ": ",sep=""))
  cat(colnames(NewsResultPam$medoids)[which(NewsResultPam$medoids[i,]==i)], "\n")
  # print tweets in cluster i
  print(rdmtweets[NewsResultPam$clustering==i])
}



colnames(NewsResultPam$medoids)[which(NewsResultPam$clustering==1)]
colnames(NewsResultPam$medoids)[NewsResultPam$clustering==1]


plot(NewsResultPam)

#generate cluster wise wordcloud
layout(matrix(c(1,2,3,4),2,2))
for(k in 1:4){
  cl<-which(NewsFit_pamK$pamobject$clustering==k)
  tdm<-t(dtm_Train_TFIDF_sp[cl,])
  v<-sort(rowSums(tdm),decreasing=TRUE)
  d=data.frame(words=names(v),freq=v)
}

wordcloud(d$words,d$freq,min.freq = 5,rot.per=0.2,random.order = FALSE,colors = brewer.pal(8,"Dark2"))
